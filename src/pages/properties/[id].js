...
export default function Properties({ pageProps: { property } }) {
      ...
      <div className="mt-4">
        <ul role="list" className="list-disc space-y-2 pl-4 text-sm">
          {property.highlights.map((highlight) => (
            ...
            <div className="mt-10">
              <h2 className="text-sm font-medium text-gray-900">Detalhes da propriedade</h2>
...
export async function getStaticPaths() {
  ...
  const { data } = await axios.get(`${URL}/api/properties/getall`);
...
export async function getStaticProps({ params: { id } }) {
  ...
  const { data } = await axios.get(`${URL}/api/properties/id/${id}`);
